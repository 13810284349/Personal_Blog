...
- 用关键词搜索昵称、正文和文章 slug；完整 UUID 按评论 ID 精确查找。
...
| `/api/admin/comments` | GET | Bearer token | `status`、`q` query | 读取指定状态评论，可按关键词或评论 ID 筛选 |
...
sequenceDiagram
  ...
  AF->>AF: requireAdmin()
  AF->>DB: select pending comments filtered by keyword or id
  DB-->>AF: comments
  AF-->>A: comments
